...
function SimplePaymentRightActions({ isProfileDropdownOpen, setIsProfileDropdownOpen, navigate, logout, identity, handleLogin }) {
  ...
  const principalText = identity && !identity.getPrincipal().isAnonymous() ? identity.getPrincipal().toText() : "";
  ...
  if (!identity || identity.getPrincipal().isAnonymous()) {
    return (
      <button onClick={handleSignIn} disabled={isLoading} className="flex items-center gap-2 bg-gradient-to-r from-[#C6A960] to-[#D4B76E] hover:opacity-90 disabled:opacity-50 px-4 sm:px-6 py-2 sm:py-2.5 rounded-full transition-opacity font-medium text-white text-sm sm:text-base">
        {isLoading ? (
          <>
          ...
          <>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 sm:w-5 sm:h-5">
              ...
              <path d="M6 21c0-3.314 2.686-6 6-6s6 2.686 6 6" stroke="white" strokeWidth="2" strokeLinecap="round" />
            </svg>
  ...
  return (
    <div className="relative profile-dropdown">
      <button onClick={() => setIsProfileDropdownOpen(!isProfileDropdownOpen)} className="group flex items-center gap-3 p-1.5 pr-3 rounded-full hover:bg-gray-100 transition-colors duration-200 ease-out cursor-pointer">
        {/* Icon */}
        ...
        <div className="flex flex-col items-start">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-xs font-mono text-gray-500">{truncateAddress(principalText)}</span>
        ...
        {/* Chevron Icon */}
        <svg className="w-5 h-5 text-yellow-900/70 transition-transform duration-300 ease-out" style={{ transform: isProfileDropdownOpen ? "rotate(180deg)" : "rotate(0deg)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <AnimatePresence>
        {isProfileDropdownOpen && (
          <motion.div className="absolute top-full right-0 mt-3 w-[270px] rounded-2xl font-normal border border-gray-200 z-[9999] overflow-hidden bg-white shadow-xl" initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} transition={{ duration: 0.2, ease: "easeOut" }}>
            <div className="py-4">
              {/* Address Section */}
              {principalText && (
                <>
                  <div className="mx-5 mb-3">
                    ...
                    <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg border border-gray-100">
                      <p className="text-xs font-mono text-gray-700 flex-1 break-all">{principalText}</p>
                      <button onClick={handleCopyAddress} className="p-1.5 hover:bg-gray-200 rounded transition-colors flex-shrink-0" title="Copy address">
  ...
  return (
    <div className="relative flex flex-col md:flex-row min-h-screen bg-white w-full">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-10 w-full flex items-center justify-between px-4 py-4 border-b border-gray-200 bg-white/95 backdrop-blur-sm">
        <Link to="/">
          <img src="/assets/logo/fradium-paylink-light.svg" alt="Fradium Logo Paylink" className="h-10" />
        </Link>
        <SimplePaymentRightActions isProfileDropdownOpen={isProfileDropdownOpen} setIsProfileDropdownOpen={setIsProfileDropdownOpen} navigate={navigate} logout={logout} identity={identity} handleLogin={handleLogin} />
      </div>
      ...
      <aside className="fixed top-0 left-0 z-[10000] w-[200px] lg:w-[240px] xl:w-[300px] bg-transparent flex-col py-8 pl-5 lg:pl-7 xl:pl-8 hidden md:flex h-screen">
        <Link to="/" className="flex items-center gap-3">
          <img src="/assets/logo/fradium-paylink-light.svg" alt="Fradium Logo Paylink" className="" />
        </Link>
      </aside>
