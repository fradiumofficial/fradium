...
export default function AnalysisProgressModal({ open, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
            <div className="bg-[#23272F] rounded-2xl px-8 py-10 w-full max-w-sm flex flex-col items-center shadow-lg relative overflow-hidden">
                ...
                <div className="text-[#B0B6BE] text-sm text-center space-y-1 z-10">
                    <div>Check if this address Already Flagged...</div>
                    <div>Analyzing Address with AI...</div>
                    <div>Analyzing Transaction Patterns...</div>
                    <div>Checking Transaction History...</div>
                    <div className="opacity-40">Cross-referencing with Known Threats</div>
                </div>
            </div>
        </div>
    );
