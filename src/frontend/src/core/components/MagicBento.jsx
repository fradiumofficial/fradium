...
const MagicBento = ({ children, textAutoHide = false, enableStars = true, enableSpotlight = true, enableBorderGlow = false, enableTilt = true, enableMagnetism = true, clickEffect = true, spotlightRadius = 250, particleCount = 10, glowColor = "132, 0, 255", className = "" }) => {
  ...
  useEffect(() => {
    ...
    const handleEnter = () => {
      ...
      // no movement effects (removed scale, purple glow, and lift)
      if (hoverOverlayRef.current) {
        gsap.to(hoverOverlayRef.current, { opacity: 0, duration: 0.18, ease: "power2.out" });
      }
      if (enableStars) {
    ...
    <div
      ...
      <div
        ...
        style={
          enableBorderGlow
            ? {
                boxShadow: "0 0 0 1px rgba(255,255,255,0.08), 0 0 24px var(--glow), inset 0 0 24px rgba(255,255,255,0.04)",
              }
            : undefined
        }>
        <div className={textAutoHide ? "transition-opacity duration-150 hover:opacity-80" : ""}>{children}</div>
