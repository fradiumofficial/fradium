                ...
                {swapQuote && !error && (
                  <div className="bg-[#23272F] border border-[#393E4B] rounded-lg p-4 space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white font-medium">{parseFloat(fromAmount).toFixed(6)} {fromToken.symbol}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-white/70">You'll Receive ~</span>
                      <span className="text-green-400 font-medium">{swapQuote.estimatedOutput.toFixed(6)} {toToken.symbol}</span>
                    </div>
                    <div className="border-t border-white/10 my-2 pt-2"></div>
                    <div className="flex justify-between text-xs">
                      <span className="text-white/50">Exchange Rate</span>
                      <span className="text-white/70">1 {fromToken.symbol} = {swapQuote.rate.toFixed(6)} {toToken.symbol}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-white/50">Pool Fee (0.3%)</span>
                      <span className="text-white/70">{swapQuote.fee.toFixed(6)} {fromToken.symbol}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-white/50">Minimum Received</span>
                      <span className="text-white/70">{swapQuote.minAmountOut.toFixed(6)} {toToken.symbol}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-white/50">Pool Status</span>
                      <span className={`text-xs ${swapQuote.hasLiquidity ? 'text-green-400' : 'text-orange-400'}`}>
