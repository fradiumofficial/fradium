...
export default function P2PTradePage() {
  ...
  const getFilteredEscrows = () => {
    return escrows.filter((escrow) => {
      // Filter out invited trades - only show open trades
      const isInvitedTrade = Array.isArray(escrow.recipient) ? escrow.recipient[0] : escrow._recipient;

      if (isInvitedTrade) {
        return false; // Hide invited trades
      }
      ...
      <div className="w-full">
        ...
        {error && (
          <div className="flex flex-col items-center justify-center py-8 gap-3">
            ...
            <button onClick={() => fetchOpenEscrows(0, false)} className="px-4 py-2 bg-[#23272F] border border-[#393E4B] hover:bg-[#2A2F37] hover:border-[#7C72FE] rounded-lg text-white text-sm transition-colors">
              Try Again
            </button>
          </div>
        )}
            ...
            {getFilteredEscrows().map((escrow, idx) => (
              <div key={escrow.escrow_id}>
                <EscrowCard
                  escrow={escrow}
                  index={idx}
                  isExpanded={expandedItems.has(escrow.escrow_id)}
