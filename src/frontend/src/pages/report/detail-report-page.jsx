...
export default function DetailReportPage() {
  ...
  const convertBackendDataToUI = (backendData) => {
    ...
    const voteDeadline = new Date(
      parseInt(backendData.vote_deadline) / 1000000
    );
    ...
    const reporterString =
      typeof backendData.reporter === "object" && backendData.reporter._arr
        ? backendData.reporter.toString()
        : String(backendData.reporter);
    ...
    const hasUserVoted = backendData.voted_by.some((voter) => {
      const voterString =
        typeof voter.voter === "object" && voter.voter._arr
    ...
    return {
      ...
      riskLevel:
        backendData.category.charAt(0).toUpperCase() +
        backendData.category.slice(1),
    };
  };
  ...
  const handleImageNavigation = (direction) => {
    ...
    if (direction === "next") {
      ...
      setCurrentImageIndex(
        (prev) => (prev - 1 + uiData.evidence.length) % uiData.evidence.length
      );
    }
  };
            ...
            <div className="xl:col-span-2 space-y-6 sm:space-y-8">
              ...
              <div className="flex flex-col lg:flex-row lg:items-start justify-between mb-6 space-y-4 lg:space-y-0">
                ...
                <div className="text-left lg:text-right">
                  ...
                  <div className="font-mono text-lg">
                    #{uiData.id.toString().padStart(4, "0")}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-red-500/10 to-red-600/10 border border-red-400/20 rounded-xl p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-4 sm:space-y-0">
                  ...
                  <Button
                    onClick={() => copyToClipboard(uiData.address)}
              ...
              {/* Description */}
              <h2 className="text-xl sm:text-2xl font-semibold mb-6 mt-10">
                Threat Analysis
              </h2>
                  ...
                  <div className="mb-2">
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-red-400">
                        Unsafe: {uiData.yesPercentage}%
                      </span>
                      <span className="text-green-400">
                        Safe: {uiData.noPercentage}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-700 rounded-full h-2">
                      <div
                        className="bg-red-400 h-2 rounded-full transition-all duration-300"
                        style={{ width: `${uiData.yesPercentage}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
                ...
                {uiData.status === "Pending" && !isAuthenticated && (
                  <div className="text-center py-4 px-6">
                    <p className="text-gray-400 text-sm mb-3">
                      Please login to vote on this report
                    </p>
                    <PrimaryButton
                      className="w-full"
                      onClick={() => handleLogin()}
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-sm">
                        {uiData.createdAt.toLocaleDateString()}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {getExplorerName(uiData.chain) !== "Explorer" && (
                <>
                  <h3 className="text-lg font-semibold mb-4 flex items-center">
                    <span className="mr-2">
                      {getExplorerIcon(uiData.chain)}
                    </span>
                    {getExplorerName(uiData.chain)}
                  </h3>
                  <a
                    href={getExplorerUrl(uiData.chain, uiData.address)}
                    target="_blank"
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="0"
                    min="0"
                    max={userBalance ? convertE8sToToken(userBalance) : 0}
