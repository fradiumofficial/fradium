  ...
  return (
    <div className="flex flex-col gap-8 max-w-xl mx-auto w-full bg-[#0F1219]">
      ...
      <div className="flex flex-col gap-4">
        <h1 className="text-white text-2xl font-semibold">Transaction Historys</h1>
            ...
            <div key={transaction.id}>
              <div className="flex items-center justify-between py-4">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className={`px-3 py-1 rounded-xl text-sm font-medium ${transaction.status === "Completed" ? "text-[#9BE4A0] bg-[#9BE4A0] bg-opacity-20" : "text-white bg-white bg-opacity-20"}`}>{transaction.status}</div>
                </div>
              </div>
              {/* Divider */}
              {index < transactions.length - 1 && <div className="border-b border-[#23272F]"></div>}
            </div>
          ))}
        </div>
      </div>
