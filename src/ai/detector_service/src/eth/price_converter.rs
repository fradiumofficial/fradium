// src/eth/price_converter.rs

use super::config::{
    CRYPTOCOMPARE_API_KEY, CRYPTOCOMPARE_URL, DEFILLAMA_URL, HTTP_OUTCALL_CYCLES,
    MORALIS_API_KEY, MORALIS_METADATA_URL, MORALIS_PRICE_URL,
};
use super::models::{logs, DefiLlamaResponse, MoralisPrice, MoralisTokenMetadata, TokenInfo};
use crate::STATE; // <-- Import the global state
...
pub async fn get_token_info(token_address: &str) -> Result<TokenInfo, String> {
    ...
    if let Some(info) = STATE.with(|s| s.borrow().token_info_cache.get(&lower_address).cloned()) {
        return Ok(info);
    }
    ...
    if let Some(info) = token_map.get(lower_address.as_str()) {
        STATE.with(|s| {
            s.borrow_mut()
                .token_info_cache
                .insert(lower_address.clone(), info.clone())
        });
        ...
        Ok((response,)) => {
            ...
            if status_code == 200 {
                ...
                if let Ok(metadata_vec) = data {
                    ...
                    } else { TokenInfo { symbol: "UNKNOWN".to_string(), decimals: 18 } }
                } else {
    ...
    STATE.with(|s| s.borrow_mut().token_info_cache.insert(lower_address, info.clone()));
    Ok(info)
}
...
pub async fn get_token_eth_ratio(token_address: &str, timestamp: u64) -> Result<f64, String> {
    ...
    if let Some(ratio) = STATE.with(|s| s.borrow().token_price_cache.get(&cache_key).cloned()) {
        return Ok(ratio);
    }
    ...
    if ratio > 0.0 {
        STATE.with(|s| s.borrow_mut().token_price_cache.insert(cache_key, ratio));
    }
...
pub async fn get_eth_btc_ratio(timestamp: u64) -> Result<f64, String> {
    ...
    if let Some(price) = STATE.with(|s| s.borrow().price_cache.get(&monthly_key).cloned()) {
        return Ok(price);
    }
    ...
    if price > 0.0 {
        STATE.with(|s| s.borrow_mut().price_cache.insert(monthly_key, price));
        Ok(price)
    } else {
        let year = dt.year();
