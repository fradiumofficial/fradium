export const idlFactory = ({ IDL }) => {
  ...
  const GetBlockParams = IDL.Record({
    ...
    'commitment' : IDL.Opt(
      IDL.Variant({ 'finalized' : IDL.Null, 'confirmed' : IDL.Null })
    ),
  });
  ...
  const TransactionStatusMeta = IDL.Record({
    ...
    'returnData' : IDL.Opt(
      IDL.Record({ 'data' : IDL.Text, 'programId' : Pubkey })
    ),
  ...
  const EncodedTransaction = IDL.Variant({
    ...
    'binary' : IDL.Tuple(
      IDL.Text,
      IDL.Variant({ 'base58' : IDL.Null, 'base64' : IDL.Null }),
    ),
  });
  const EncodedTransactionWithStatusMeta = IDL.Record({
    ...
    'version' : IDL.Opt(
      IDL.Variant({ 'legacy' : IDL.Null, 'number' : IDL.Nat8 })
    ),
  });
  ...
  const MultiGetRecentPrioritizationFeesResult = IDL.Variant({
    ...
    'Inconsistent' : IDL.Vec(
      IDL.Tuple(RpcSource, GetRecentPrioritizationFeesResult)
    ),
  });
  ...
  const MultiGetSignaturesForAddressResult = IDL.Variant({
    ...
    'Inconsistent' : IDL.Vec(
      IDL.Tuple(RpcSource, GetSignaturesForAddressResult)
    ),
  });
  ...
  const MultiGetTokenAccountBalanceResult = IDL.Variant({
    ...
    'Inconsistent' : IDL.Vec(
      IDL.Tuple(RpcSource, GetTokenAccountBalanceResult)
    ),
  });
  ...
  return IDL.Service({
    ...
    'getProviders' : IDL.Func(
        [],
        [IDL.Vec(IDL.Tuple(SupportedProvider, RpcProvider))],
        ['query'],
      ),
    ...
    'updateApiKeys' : IDL.Func(
        [IDL.Vec(IDL.Tuple(SupportedProvider, IDL.Opt(IDL.Text)))],
        [],
        [],
      ),
  });
