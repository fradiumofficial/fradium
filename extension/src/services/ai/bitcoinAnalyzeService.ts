...
/**
 * Adapts transactions from Mempool Space to format suitable for AnalyzeAddress model.
 * @param tx Transaction from Mempool Space.
 * @returns Adapted transaction.
 */
...
/**
 * Fetches transactions for a specific Bitcoin address.
 * Data will be fetched directly from Mempool Space API.
 * @param address Bitcoin address to analyze.
 * @returns List of transactions associated with the address.
 */
async function fetchTransactions(address: string) {
  ...
  try {
    ...
    // Add timeout and better error handling for extension environment
    ...
    console.error(`Bitcoin transaction fetch failed: ${error}`);
    
    // Return empty transactions array as fallback instead of throwing
    console.warn('Returning empty transaction set for Bitcoin analysis');
    return [];
  }
}

/**
 * Extracts feature vector from list of transactions according to order required by AI model.
 * @param transactions Array of transactions from `fetchTransactions`.
 * @param targetAddress Bitcoin address being analyzed.
 * @returns Array of numbers (feature vector) ready to send to Canister.
 */
export async function extractFeatures(targetAddress: string): Promise<number[]> {
  ...
  for (const key in btcTransactedStats) {
    features[`btc_transacted_${key}`] = (btcTransactedStats as any)[key];
  }

  const btcSentStats = calculateStats(sentValuesBtc);
  for (const key in btcSentStats) {
    features[`btc_sent_${key}`] = (btcSentStats as any)[key];
  }

  const btcReceivedStats = calculateStats(receivedValuesBtc);
  for (const key in btcReceivedStats) {
    features[`btc_received_${key}`] = (btcReceivedStats as any)[key];
  }

  const feesStats = calculateStats(allFeesBtc);
  for (const key in feesStats) {
    features[`fees_${key}`] = (feesStats as any)[key];
  }
  ...
  for (const key in feeSharesStats) {
    features[`fees_as_share_${key}`] = (feeSharesStats as any)[key];
  }

  const blockIntervals = calculateIntervals(blockHeights);
  const blockIntervalsStats = calculateStats(blockIntervals);
  for (const key in blockIntervalsStats) {
    features[`blocks_btwn_txs_${key}`] = (blockIntervalsStats as any)[key];
  }

  const sentBlockIntervals = calculateIntervals(sentBlocks);
  const sentBlockIntervalsStats = calculateStats(sentBlockIntervals);
  for (const key in sentBlockIntervalsStats) {
    features[`blocks_btwn_input_txs_${key}`] = (sentBlockIntervalsStats as any)[key];
  }
  ...
  for (const key in receivedBlockIntervalsStats) {
    features[`blocks_btwn_output_txs_${key}`] = (receivedBlockIntervalsStats as any)[key];
  }
  ...
  for (const key in interactionStats) {
    features[`transacted_w_address_${key}`] = (interactionStats as any)[key];
  }
  ...
  // 3. Calculate enhanced pattern features
  ...
  // 4. Arrange feature vector in CORRECT ORDER
  // This order is CRITICAL and must match exactly with Rust implementation.
  const featureNames = [
    "Time step",
    "num_txs_as_sender",
    "num_txs_as_receiver",
...
const calculateIntervals = (blocks: number[]) => {
