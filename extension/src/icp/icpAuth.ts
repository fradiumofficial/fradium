import { AuthClient } from "@dfinity/auth-client";

export function getInternetIdentityProvider(): string {
    // Vite envs are on import.meta.env
    const network = (import.meta as any).env?.DFX_NETWORK;
    const canisterId = (import.meta as any).env?.CANISTER_ID_INTERNET_IDENTITY;

    if (network === 'local' && canisterId) {
        return `http://${canisterId}.localhost:4943`;
    }
    return 'https://identity.ic0.app';
}

export function shortPrincipal(principalText?: string): string {
    if (!principalText) return 'Au...Ux';
    if (principalText.length <= 10) return principalText;
    return `${principalText.slice(0, 2)}...${principalText.slice(-2)}`;
}

export async function loginWithInternetIdentity(): Promise<string> {
    const identityProvider = getInternetIdentityProvider();
    
    // Use consistent AuthClient with proper storage configuration
    const client = await getAuthClient();

    const windowFeatures = [
        "width=500",
        "height=600",
        "scrollbars=yes",
        "resizable=yes",
        "toolbar=no",
        "menubar=no",
        "location=no",
        "status=no",
        "directories=no",
    ].join(",");

    await new Promise<void>((resolve, reject) =>
        client.login({
            identityProvider,
            onSuccess: () => resolve(),
            onError: (err) => reject(err),
            windowOpenerFeatures: windowFeatures,
            maxTimeToLive: BigInt(8 * 60 * 60 * 1000 * 1000 * 1000), // 8 hours in nanoseconds
        })
    );

    const principal = client.getIdentity().getPrincipal().toText();
    try {
        localStorage.setItem("icp_principal", principal);
        localStorage.setItem("icp_authenticated", "true");
        
        localStorage.setItem("icp_session_created", Date.now().toString());
        
        console.log("ICP Auth: Stored authentication state with session info");
    } catch (e) {
        console.error("ICP Auth: Failed to store authentication state:", e);
    }
    return principal;
}

export function readStoredPrincipal(): string | null {
    try {
        return localStorage.getItem("icp_principal");
    } catch {
        return null;
    }
}

export function isStoredAuthenticated(): boolean {
    try {
        const authenticated = localStorage.getItem("icp_authenticated");
        const principal = localStorage.getItem("icp_principal");
        return authenticated === "true" && !!principal;
    } catch {
        return false;
    }
}

export function clearStoredAuth(): void {
    try {
        // Clear our custom storage
        localStorage.removeItem("icp_principal");
        localStorage.removeItem("icp_authenticated");
        localStorage.removeItem("icp_session_created");
        
        // Clear AuthClient storage keys (common ones)
        localStorage.removeItem("identity");
        localStorage.removeItem("delegation");
        
        // Clear Chrome storage as well
        if (typeof chrome !== 'undefined' && chrome.storage) {
            chrome.storage.local.remove([
                "icp_principal", 
                "icp_authenticated", 
                "icp_session_created",
                "identity", 
                "delegation"
            ]);
        }
        
        console.log("ICP Auth: Cleared all stored authentication state");
    } catch (e) {
        console.error("ICP Auth: Failed to clear authentication state:", e);
    }
}

export async function resetAuthState(): Promise<void> {
    try {
        console.log("ICP Auth: Resetting all authentication state...");
        
        // Clear storage first
        clearStoredAuth();
        
        // Create new AuthClient and logout
        const client = await AuthClient.create({
            keyType: 'Ed25519',
        });
        
        await client.logout();
        console.log("ICP Auth: Authentication state reset complete");
    } catch (e) {
        console.error("ICP Auth: Failed to reset authentication state:", e);
    }
}

export async function getAuthClient() {
    return await AuthClient.create({
        // Use Ed25519 key type for string serialization compatibility
        keyType: 'Ed25519',
        // Configure session duration
        idleOptions: {
            // Idle timeout in milliseconds (30 minutes = 30 * 60 * 1000)
            idleTimeout: 30 * 60 * 1000,
            // Whether to capture scroll/mouse/keyboard events to detect activity
            captureScroll: true,
            // Callback when session becomes idle
            onIdle: () => {
                console.log('ICP Auth: Session became idle, logging out...');
                // You can add custom logic here before logout
            }
        },
        storage: {
            get: async (key: string) => {
                try {
                    // Try localStorage first
                    const value = localStorage.getItem(key);
                    if (value !== null) {
                        return value;
                    }
                    
                    // Fallback to Chrome storage for extension environment
                    if (typeof chrome !== 'undefined' && chrome.storage) {
                        const result = await chrome.storage.local.get(key);
                        return result[key] || null;
                    }
                    
                    return null;
                } catch {
                    return null;
                }
            },
            set: async (key: string, value: string) => {
                try {
                    // Set in localStorage
                    localStorage.setItem(key, value);
                    
                    // Also set in Chrome storage for persistence
                    if (typeof chrome !== 'undefined' && chrome.storage) {
                        await chrome.storage.local.set({ [key]: value });
                    }
                } catch {
                    // ignore
                }
            },
            remove: async (key: string) => {
                try {
                    // Remove from localStorage
                    localStorage.removeItem(key);
                    
                    // Also remove from Chrome storage
                    if (typeof chrome !== 'undefined' && chrome.storage) {
                        await chrome.storage.local.remove(key);
                    }
                } catch {
                    // ignore
                }
            }
        }
    });
}

// Session duration utility functions
export interface SessionInfo {
    isValid: boolean;
    createdAt: Date | null;
    expiresAt: Date | null;
    remainingTime: number; // in milliseconds
    remainingTimeFormatted: string;
}

/**
 * Get current session information including duration and expiry
 */
export async function getSessionInfo(): Promise<SessionInfo> {
    const defaultInfo: SessionInfo = {
        isValid: false,
        createdAt: null,
        expiresAt: null,
        remainingTime: 0,
        remainingTimeFormatted: 'Session expired'
    };

    try {
        const client = await getAuthClient();
        const isAuthenticated = await client.isAuthenticated();
        
        if (!isAuthenticated) {
            return defaultInfo;
        }

        // Get session creation time from localStorage
        const sessionCreatedStr = localStorage.getItem("icp_session_created");
        if (!sessionCreatedStr) {
            return { ...defaultInfo, isValid: true, remainingTimeFormatted: 'Unknown session time' };
        }

        const sessionCreated = new Date(parseInt(sessionCreatedStr));
        const now = new Date();
        
        // Internet Identity sessions typically last 8 hours maximum
        const maxSessionDuration = 8 * 60 * 60 * 1000; // 8 hours in milliseconds
        const sessionExpiry = new Date(sessionCreated.getTime() + maxSessionDuration);
        
        const remainingTime = sessionExpiry.getTime() - now.getTime();
        const isValid = remainingTime > 0;

        return {
            isValid,
            createdAt: sessionCreated,
            expiresAt: sessionExpiry,
            remainingTime: Math.max(0, remainingTime),
            remainingTimeFormatted: isValid ? formatDuration(remainingTime) : 'Session expired'
        };
    } catch (error) {
        console.error('Error getting session info:', error);
        return defaultInfo;
    }
}

/**
 * Format duration in milliseconds to human readable string
 */
function formatDuration(ms: number): string {
    if (ms <= 0) return 'Expired';
    
    const hours = Math.floor(ms / (1000 * 60 * 60));
    const minutes = Math.floor((ms % (1000 * 60 * 60)) / (1000 * 60));
    const seconds = Math.floor((ms % (1000 * 60)) / 1000);
    
    if (hours > 0) {
        return `${hours}h ${minutes}m`;
    } else if (minutes > 0) {
        return `${minutes}m ${seconds}s`;
    } else {
        return `${seconds}s`;
    }
}

/**
 * Check if session is about to expire (within 5 minutes)
 */
export async function isSessionNearExpiry(): Promise<boolean> {
    const sessionInfo = await getSessionInfo();
    const fiveMinutes = 5 * 60 * 1000; // 5 minutes in milliseconds
    return sessionInfo.isValid && sessionInfo.remainingTime < fiveMinutes;
}

/**
 * Extend session by logging in again (if needed)
 */
export async function extendSession(): Promise<boolean> {
    try {
        const sessionInfo = await getSessionInfo();
        if (!sessionInfo.isValid) {
            console.log('Session expired, need to login again');
            return false;
        }
        
        // If session is near expiry, we could implement automatic refresh
        // For Internet Identity, this typically requires user interaction
        const client = await getAuthClient();
        const isStillAuthenticated = await client.isAuthenticated();
        
        if (isStillAuthenticated) {
            // Update session created time to current time
            localStorage.setItem("icp_session_created", Date.now().toString());
            console.log('Session extended successfully');
            return true;
        }
        
        return false;
    } catch (error) {
        console.error('Error extending session:', error);
        return false;
    }
}
