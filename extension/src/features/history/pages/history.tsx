...
function History() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (solAddr)
          tasks.push(getSolanaTransactionHistory(solAddr, "devnet", 30))
        if (icpPrincipal)
          tasks.push(
            getICRCTransactionHistory("icp", icpPrincipal, icpAccount, 30)
          )
        if (icpPrincipal)
          tasks.push(
            getICRCTransactionHistory("fradium", icpPrincipal, null, 30)
          )
        if (icpPrincipal)
          tasks.push(getCkBtcTransactionHistory(icpPrincipal, 30))
        if (icpPrincipal)
          tasks.push(getCkEthTransactionHistory(icpPrincipal, 30))

        const results = await Promise.allSettled(tasks)
        const onchain = results.flatMap((r) =>
  ...
  const formatAmount = (
    ...
    return {
      text: `${sign} ${tx.amount} ${tx.tokenType === "Bitcoin" ? "BTC" : tx.tokenType === "Solana" ? "SOL" : tx.tokenType}`,
      color
    }
  }
  ...
  return (
    <div className="w-[375px] text-white flex flex-col">
      <div
        className={`relative flex-1 ${SHOW_EMPTY ? "overflow-hidden" : "overflow-y-auto"}`}>
            ...
            {isLoading ? (
              ...
              <div className="relative z-10 w-full h-full flex items-center justify-center text-center">
                <div>
                  ...
                  <div className="text-red-400 text-[14px] max-w-[320px] mx-auto mb-3">
                    {error}
                  </div>
                  <button
                    type="button"
              ...
              <div className="relative z-10 w-full h-full flex items-center justify-center text-center">
                <div>
                  ...
                  <div className="text-white/60 text-[14px] font-normal leading-relaxed max-w-[320px] mx-auto">
                    Use your fradium wallet to send, receive tokens - and your
                ...
                {filtered.map((tx) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        navigate(ROUTES.TX_DETAIL.replace(":id", tx.id))
                      }>
                      <div className="flex items-start justify-between">
                        <div className="flex items-center">
                          <img
                            src={getIcon(tx.tokenType)}
                            alt={tx.tokenType}
