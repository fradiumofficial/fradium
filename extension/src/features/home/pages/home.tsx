...
function Home() {
    ...
    if (selectedNetwork === "all") {
      ...
      tokens = tokens.filter((token) => {
        switch (token.networkKey) {
          case "btc":
            return networkFilters?.Bitcoin ?? true
          case "eth":
            return networkFilters?.Ethereum ?? true
          case "sol":
            return networkFilters?.Solana ?? true
          case "fra":
            return networkFilters?.["Internet Computer"] ?? true
          case "icp":
            return networkFilters?.["Internet Computer"] ?? true
          case "ckbtc":
            return networkFilters?.["Internet Computer"] ?? true
      ...
      const targetNetwork =
        networkMap[selectedNetwork as keyof typeof networkMap]
      if (targetNetwork) {
  ...
  const formatTokenBalance = useCallback(
    (tokenId: string, balance: string) => {
      ...
      if (numericBalance < smallThreshold) {
        const th = smallThreshold.toLocaleString("en-US", {
  ...
  const hasAnyBalanceLoaded = useMemo(() => {
    return (extensionTokens || []).some(
      (t: any) => balances?.[t.id] !== undefined
    )
  ...
  React.useEffect(() => {
    ...
    const anyLoaded = (extensionTokens || []).some(
      (t: any) => balances?.[t.id] !== undefined
    )
    const anyLoading = (extensionTokens || []).some(
      (t: any) => balanceLoading?.[t.id]
    )
  ...
  const formatUSDDisplay = useCallback(
    (value: number) => {
      if (hideBalance) return "••••"
            ...
            <div className="flex flex-col items-center p-0 gap-[4px] w-full h-[60px] flex-none order-1 flex-grow-0">
              ...
              <div className="flex flex-row items-center justify-center gap-2 w-full flex-none order-1 flex-grow-0 text-center">
                {isInitialLoading ? (
                  ...
                  (() => {
                    const tokensWithValue = usdBreakdown.filter(
                      ...
                      tokensWithValue
                        ...
                        .map((item, index) => (
                          <div
                            ...
                            {index <
                              Math.min(tokensWithValue.length, 3) - 1 && (
          ...
          <div className="flex flex-row items-start p-0 gap-2 w-[303px] h-[45px] flex-none order-1 self-stretch flex-grow-0">
            ...
            <button
              ...
              <div className="flex flex-row items-center p-0 gap-[6px] mx-auto w-[126px] h-[21px] flex-none order-0 flex-grow-0">
                <div className="w-5 h-5 flex-none order-0 flex-grow-0 relative">
                  <img
                    ...
                    alt="QR Code"
                    className="w-[15px] h-[15px] absolute left-[calc(50%-15px/2)] top-[calc(50%-15px/2)]"
                  />
                </div>
              ...
              <div className="mx-auto w-5 h-5 flex-none order-1 flex-grow-0 relative">
                <img
                  ...
                  alt="Receive"
                  className="w-[11.67px] h-[11.67px] absolute left-[calc(50%-11.67px/2+51.92px)] top-[calc(50%-11.67px/2+0.17px)] rotate-180"
                />
              </div>
            </button>
            ...
            <button
              ...
              <div className="flex flex-row items-center p-0 gap-[6px] mx-auto w-[126px] h-[21px] flex-none order-0 flex-grow-0">
                <div className="w-5 h-5 flex-none order-0 flex-grow-0 relative">
                  <img
                    src={CDN.icons.sendToken}
                    alt="Send"
                    className="w-[16.67px] h-[16.67px] absolute left-[calc(50%-16.67px/2+0px)] top-[calc(50%-16.67px/2+0px)]"
                  />
                </div>
                <div className="w-[100px] h-[21px] font-['General Sans'] font-medium text-[16px] leading-[130%] text-white flex-none order-1 flex-grow-0">
                  Send
