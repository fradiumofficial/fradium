...
export default function SendConfirm() {
  ...
  const tokenMeta = useMemo(() => {
    const map: Record<string, { symbol: string; tokenType: string }> = {
      ...
      fra: { symbol: "FRADIUM", tokenType: "Fradium" },
      ...
      fradium: { symbol: "FRADIUM", tokenType: "Fradium" }
    }
    return map[selectedNetwork] || map["btc"]
  }, [selectedNetwork])

  const convertToUnits = useCallback(
    ...
    try {
      ...
      switch (selectedNetwork) {
        ...
        case "bitcoin":
          ...
          txResult = await walletActor.bitcoin_send({
            destination_address: recipientAddress,
            amount_in_satoshi: convertToUnits(amount)
          })
          break
        case "eth":
        case "ethereum":
          if (!walletActor) throw new Error("Wallet actor not available")
          txResult = await walletActor.ethereum_send(
            recipientAddress,
            convertToUnits(amount)
          )
          break
        case "sol":
        case "solana":
          if (!walletActor) throw new Error("Wallet actor not available")
          txResult = await walletActor.solana_send(
            recipientAddress,
            convertToUnits(amount)
          )
          break
        case "icp": {
          const res = await sendIcrcTransfer(
            "icp",
            recipientAddress,
            parseFloat(amount)
          )
          if (!res?.success)
            throw new Error(res?.error || "ICP transfer failed")
        ...
        case "fradium": {
          ...
          if (!res?.success)
            throw new Error(res?.error || "Fradium transfer failed")
          txResult = res?.blockIndex || ""
          break
        }
        case "ckbtc": {
          ...
          if (!res?.success)
            throw new Error(res?.error || "ckBTC transfer failed")
          txResult = res?.blockIndex || ""
          break
        }
        case "cketh": {
          ...
          if (!res?.success)
            throw new Error(res?.error || "ckETH transfer failed")
          txResult = res?.blockIndex || ""
          break
        }
        default:
