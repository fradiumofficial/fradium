...
const AnalyzerIcon: React.FC<{ className?: string; isActive?: boolean }> = ({ isActive, ...props }) => (
...
const HistoryIcon: React.FC<{ className?: string; isActive?: boolean }> = ({ isActive, ...props }) => (
  <svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
  </svg>
);

const AccountIcon: React.FC<{ className?: string; isActive?: boolean }> = ({ isActive, ...props }) => (
  <svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
        ...
        return (
          ...
          >
            ...
            <div className="flex flex-col items-center gap-1 w-6 h-6">
              {item.id === 'Wallet' ? (
                <WalletIcon isActive={isActive} className="transition-all duration-300 ease-in-out hover:scale-110" />
              ) : (
                <Icon
                  className={`w-6 h-6 transition-all duration-300 ease-in-out hover:scale-110 ${
                    isActive ? 'text-[#99E39E] drop-shadow-sm' : 'text-white/50'
                  }`}
                />
              )}
            </div>
