...
async function getAllTransactions(address: string, { etherscanApiKey }: { etherscanApiKey: string }) {
...
// ----------------------
async function getTokenInfo(tokenAddress: string, { moralisApiKey }: { moralisApiKey: string }) {
...
async function getStablecoinEthRatio(timestamp: number, { cryptocompareApiKey }: { cryptocompareApiKey: string }) {
...
async function fetchFromDefiLlama(tokenAddress: string, timestamp: number, { cryptocompareApiKey }: { cryptocompareApiKey: string }) {
...
async function fetchFromMoralisPrice(tokenAddress: string, timestamp: number, { moralisApiKey, cryptocompareApiKey }: { moralisApiKey: string, cryptocompareApiKey: string }) {
  try {
...
async function fetchTokenPriceFromApis(tokenSymbol: string, tokenAddress: string, timestamp: number, { cryptocompareApiKey, moralisApiKey }: { cryptocompareApiKey: string, moralisApiKey: string }) {
...
async function getTokenEthRatio(tokenAddress: string, timestamp: number, apis: { cryptocompareApiKey: string, moralisApiKey: string }) {
  const info = await getTokenInfo(tokenAddress, { moralisApiKey: apis.moralisApiKey });
  ...
  const ratio = ["USDT", "USDC", "DAI"].includes(symbol) ? await getStablecoinEthRatio(timestamp, { cryptocompareApiKey: apis.cryptocompareApiKey }) : await fetchTokenPriceFromApis(symbol, tokenAddress, timestamp, apis);

  if (ratio > 0) tokenPriceCache.set(cacheKey, ratio);
  return ratio;
}

async function getEthBtcRatio(timestamp: number, { cryptocompareApiKey }: { cryptocompareApiKey: string }) {
...
export async function extractFeatures(targetAddress: string, apis: any = {}) {
  ...
  console.log("- Etherscan API Key:", etherscanApiKey ? `${etherscanApiKey.substring(0, 8)}...` : "NOT SET");
  console.log("- CryptoCompare API Key:", cryptocompareApiKey ? `${cryptocompareApiKey.substring(0, 8)}...` : "NOT SET");
...
export function buildFeatureVector(featureNames: string[], featureMap: { [key: string]: any }) {
...
export function getTxCountFromFeatures(featureMap: { [key: string]: any }) {
