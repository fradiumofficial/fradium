// Solana Data Extractor Service (TypeScript port of frontend implementation)
...
// Program sets (synced with Rust config)
...
const KNOWN_TOKENS: Map<string, { symbol: string; decimals: number; name: string }> = new Map([
...
function normalizeTokenAmount(raw: number, decimals: number): number {
...
class SolanaPriceConverter {
  private tokenInfoCache = new Map<string, { symbol: string; decimals: number; name: string }>();
  private priceCache = new Map<string, number>();

  async getTokenInfo(mint: string): Promise<{ symbol: string; decimals: number; name: string }> {
    const key = normalizeAddress(mint);
    if (this.tokenInfoCache.has(key)) return this.tokenInfoCache.get(key)!;
    ...
    try {
      ...
      const res = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-api-key": process.env.PLASMO_PUBLIC_HELIUS_API_KEY },
        body,
      });
      if (res.ok) {
  ...
  async getTokenSolRatio(mint: string, timestamp: number): Promise<[number, boolean]> {
    ...
    if (["USDC", "USDT", "BUSD", "DAI"].includes(symbol)) {
      const [ratio, ok] = await getStablecoinSolRatio(timestamp, process.env.PLASMO_PUBLIC_CRYPTOCOMPARE_API_KEY);
      if (ok) {
        this.priceCache.set(cacheKey, ratio);
        return [ratio, true];
      }
    }
    let [p, ok] = await fetchTokenPriceJupiter(mint, timestamp, process.env.PLASMO_PUBLIC_CRYPTOCOMPARE_API_KEY);
    if (!ok) {
      [p, ok] = await fetchTokenPriceCryptoCompare(symbol, timestamp, process.env.PLASMO_PUBLIC_CRYPTOCOMPARE_API_KEY);
    }
...
class TransactionClassifier {
  private dexPrograms = new Set(DEX_PROGRAMS);
  private lendingPrograms = new Set(LENDING_PROGRAMS);
  private stakingPrograms = new Set(STAKING_PROGRAMS);
  private comprehensivePrograms = new Set(COMPREHENSIVE_PROGRAMS);

  classifyTransactionContext(rawTx: any): string {
    const programIds = new Set<string>(
      (rawTx.transaction?.message?.instructions || []).map((instr: any) => String(instr.program_id || ''))
    );
    if (this.hasIntersection(programIds, this.dexPrograms)) return TransactionContext.DexSwap;
    if (this.hasIntersection(programIds, this.lendingPrograms)) return TransactionContext.Lending;
    if (this.hasIntersection(programIds, this.stakingPrograms)) return TransactionContext.Staking;
    const hasTransfers = (rawTx.token_transfers?.length || 0) > 0 || (rawTx.native_transfers?.length || 0) > 0;
    if (hasTransfers && !this.hasIntersection(programIds, this.comprehensivePrograms)) return TransactionContext.PureTransfer;
    if (this.hasIntersection(programIds, this.comprehensivePrograms)) return TransactionContext.OtherProgram;
    return TransactionContext.Unknown;
  }

  isProgrammaticTransaction(rawTx: any): boolean {
    const instructionCount = (rawTx.transaction?.message?.instructions || []).length;
    if (instructionCount > 10) return true;
    if ((rawTx.token_transfers?.length || 0) > 5) return true;
    const programIds = new Set<string>(
      (rawTx.transaction?.message?.instructions || []).map((instr: any) => String(instr.program_id || ''))
    );
...
class SolanaDataExtractor {
  private priceConverter: SolanaPriceConverter;
  ...
  async parseAllTransactions(rawTransactions: any[], targetAddress: string): Promise<any[]> {
  ...
  private async fetchTransactionPage(address: string, beforeSignature: string | null): Promise<any[]> {
    let url = `${HELIUS_BASE}/v0/addresses/${address}/transactions?api-key=${process.env.PLASMO_PUBLIC_HELIUS_API_KEY}`;
    if (beforeSignature) {
      url += `&before=${encodeURIComponent(beforeSignature)}`;
    }
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        const response = await fetch(url, { headers: { "User-Agent": "Solana-Fraud-Detector/1.0", Accept: "application/json" } });
  ...
  private async parseSolanaTransaction(rawTx: any, targetAddress: string): Promise<any[]> {
  ...
  private extractSolTransfers(rawTx: any, targetAddress: string): any[] {
    let transfers: any[] = [];
    for (const transfer of rawTx.native_transfers || []) {
      ...
      if (
        (fromAddr && fromAddr.toLowerCase() === targetAddress.toLowerCase()) ||
        (toAddr && toAddr.toLowerCase() === targetAddress.toLowerCase())
      ) {
    ...
    for (const transfer of rawTx.token_transfers || []) {
      if (!isWrappedSOL(transfer.mint)) continue;
      const fromAddr = transfer.from_user_account || "";
      const toAddr = transfer.to_user_account || "";
      if (
        fromAddr.toLowerCase() === targetAddress.toLowerCase() ||
        toAddr.toLowerCase() === targetAddress.toLowerCase()
      ) {
        let amount = 0.0;
        if (transfer.raw_token_amount) {
          const rawAmount = Number(transfer.raw_token_amount);
          if (Number.isFinite(rawAmount)) {
            amount = normalizeTokenAmount(rawAmount, 9);
          }
        }
        if (amount === 0.0) {
  ...
  private async extractTokenTransfers(rawTx: any, targetAddress: string): Promise<any[]> {
    let transfers: any[] = [];
    for (const transfer of rawTx.token_transfers || []) {
      if (isWrappedSOL(transfer.mint)) continue;
...
function tsToDailyKey(ts: number): string {
...
function getDailyTimestamp(ts: number): number {
  const d = new Date(Number(ts) * 1000);
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  if (!res.ok) {
    ...
    const err: any = new Error(`HTTP ${res.status}${text ? `: ${text}` : ""}`);
...
async function getSolPriceUsd(timestamp: number, cryptocompareApiKey?: string): Promise<[number, boolean]> {
...
async function getStablecoinSolRatio(timestamp: number, cryptocompareApiKey?: string): Promise<[number, boolean]> {
  const [solUsd, ok] = await getSolPriceUsd(timestamp, cryptocompareApiKey);
  if (ok && solUsd > 0) return [1.0 / solUsd, true];
  return [0.0, false];
}

async function fetchTokenPriceJupiter(mint: string, timestamp: number, cryptocompareApiKey?: string): Promise<[number, boolean]> {
...
async function fetchTokenPriceCryptoCompare(symbol: string, timestamp: number, cryptocompareApiKey?: string): Promise<[number, boolean]> {
  const dayTs = getDailyTimestamp(timestamp);
  try {
...
async function getTokenInfo(mint: string, helHeaders: Record<string, string>): Promise<{ symbol: string; decimals: number; name: string }> {
  const key = normalizeAddress(mint);
...
async function getTokenSolRatio(mint: string, timestamp: number, cryptocompareApiKey?: string, helHeaders: Record<string, string> = {}): Promise<[number, boolean]> {
...
function addStats(obj: Record<string, any>, prefix: string, values: number[], includeTotal: boolean) {
...
function addIntervalStats(obj: Record<string, any>, prefix: string, slots: number[]) {
...
function isKnownProgram(addr: string): boolean {
  return COMPREHENSIVE_PROGRAMS.has(addr);
}

async function getSolBtcRatio(timestamp: number, cryptocompareApiKey?: string): Promise<number> {
...
export async function extractFeatures(address: string, options: any = {}): Promise<any> {
  ...
  if (!cryptocompareApiKey) {
    throw new Error("CryptoCompare API key is not configured. Set PLASMO_PUBLIC_CRYPTOCOMPARE_API_KEY and rebuild.");
  }
  if (!heliusApiKey) {
    throw new Error("Helius API key is not configured. Set VITE_HELIUS_API_KEY and rebuild.");
  }
  ...
  try {
    ...
    if (err && (err.status === 401 || String(err.message || "").includes("401"))) {
      throw new Error("Helius HTTP 401. Please check your API key.");
    }
    throw err;
  }
  ...
  let failed = 0, solTxs = 0, tokenTxs = 0, dexTxs = 0, lendingTxs = 0, stakingTxs = 0, programmaticTxs = 0, priceFailures = 0;
  ...
  for (const t of parsed) {
    ...
    switch (t.tx_type) {
      ...
      case "SolTransfer": solTxs += 1; break;
      case "TokenTransfer": tokenTxs += 1; uniqueTokens.add(t.mint_address); if (!t.price_fetch_success) priceFailures += 1; break;
      default: break;
    }
    const ctx = t.tx_context;
    if (ctx === "DEX_SWAP") dexTxs += 1; else if (ctx === "LENDING") lendingTxs += 1; else if (ctx === "STAKING") stakingTxs += 1;
  ...
  features["parsed_txs_count"] = parsed.length;
  ...
  addStats(features, "btc_sent", sent.map((t) => t.value_btc), true);
  addStats(features, "btc_received", received.map((t) => t.value_btc), true);
  addStats(features, "fees", allFeesBtc, true);
  addStats(features, "sol_sent", sent.map((t) => t.value_sol), true);
  addStats(features, "sol_received", received.map((t) => t.value_sol), true);
  ...
  addIntervalStats(features, "slots_btwn_input_txs", sentSlots.sort((a, b) => a - b));
  addIntervalStats(features, "slots_btwn_output_txs", recvSlots.sort((a, b) => a - b));
  ...
  addStats(features, "transacted_w_address", humanCounter.map(([, c]) => Number(c)), false);

  const allTxs = [...sent, ...received];
  const complexityScoreMap: Record<string, number> = { PURE_TRANSFER: 1.0, DEX_SWAP: 3.0, LENDING: 2.5, STAKING: 2.0, OTHER_PROGRAM: 2.0 };
...
export function buildFeatureVector(featureNames: string[], featureMap: Record<string, any>): number[] {
  return featureNames.map((name) => Number(featureMap[name] ?? 0.0));
}

export function getTxCountFromFeatures(featureMap: Record<string, any>): number {
...
export async function extractSolanaFeatures(address: string): Promise<SolanaFeatures> {
  return (await extractFeatures(address)) as SolanaFeatures;
}
